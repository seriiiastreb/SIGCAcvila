...
public partial class SystemSeqAdmin : System.Web.UI.Page
{
    ...
    private void Message(UpdatePanel inputPanel, int messageType, string titleWindow, string message)
    {
        InfoWindow infoBox = (InfoWindow)this.LoadControl(appPath + "/Controls/InfoWindow.ascx");
        
        infoBox.WindowTitle = titleWindow;
        infoBox.Message = message;
        infoBox.MessageType = messageType;
        inputPanel.ContentTemplateContainer.Controls.Add(infoBox);
    ...
    protected void newGroupSaveButton_Click(object sender, EventArgs e)
    {
        ...
        if (allowHere)
        {
            ...
            {
                Message(groupPermissionsConfigPanel, (int)Constants.InfoBoxMessageType.Error, "Error!", "Unable add group. Try again later."); 
            }
        }
        else
        {
    ...
    protected void deleteGroupButton_Click(object sender, EventArgs e)
    {
        ...
        if (allowHere)
        {
            if (groupsListBox.SelectedValue != null)
            {
                ...
                {
                    Message(groupPermissionsConfigPanel, (int)Constants.InfoBoxMessageType.Error, "Error!", "Unable delete group. Try again later.");
                }
            }
        }
        else
    ...
    protected void registerModulesButton_Click(object sender, EventArgs e)
    {
        ...
        if (allowHere)
        {
            ...
            {
                Message(groupPermissionsConfigPanel, (int)Constants.InfoBoxMessageType.Error, "System error!", ex.Message); 
            }
        }
        else
        {
            ...
            if (aviableModulesDDL.SelectedValue != null && aviableDomaninsListBox.SelectedValue != null && groupsListBox.SelectedValue != null)
            {
                ...
                if (viewstateDT != null)
                {
                    ...
                    if (!foundKey)
                    {
                        ...
                        {
                            Message(groupPermissionsConfigPanel, (int)Constants.InfoBoxMessageType.Error, "Error Updating record!", "Unable to updating permissions. Try again later."); 
                        }
                    }
                } 
            }                   
                ...
                if (viewstateDT != null && viewstateDT.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < viewstateDT.Rows.Count; i++)
                    {
                        if (viewstateDT.Rows[i]["display_key"].ToString().Equals(display_key))
                        {
                            ...
                            {
                                Message(groupPermissionsConfigPanel, (int)Constants.InfoBoxMessageType.Error, "Error Deleting record!", "Unable to detele permissions. Try again later."); 
                            }
                        }
                    }
    ...
    protected void usersGridView_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        try
        {
            ...
            {
                Message(usersAdminPanel, (int)Constants.InfoBoxMessageType.Warning, "Atentie", "Utilizatorul nu a fost sters. Incercati mai tirziu."); 
            }
        }
        catch (Exception ex)
        {
            Message(usersAdminPanel, (int)Constants.InfoBoxMessageType.Error, "Eroare in System!", ex.Message); 
        }        
    }
            ...
            if (!userDetailsActionHiddenField.Value.Equals(string.Empty))
            {
                ...
                if (Crypt.Module.DecodeCriptedString(userDetailsActionHiddenField.Value).Equals(Constants.UserOperation.Edit))
                {
                    if (userID != 0 && Utils.ModuleSecurity().UpdateUser(userID, nume, prenume, login, password, email, groupList, passwordStatusID, recordStatusID, userDetails_ResetPasswordCheckBox.Checked))
                    {
                        ...
                        FillUsersGridView();
                        userDetailsActionHiddenField.Value = Crypt.Module.CreateEncodedString(Constants.UserOperation.Edit);
                        Message(userDetailsPanel, (int)Constants.InfoBoxMessageType.Ok, "Congratulation", "Succesifuly updated user information.");
                    }
                    else
                    {
                        Message(userDetailsPanel, (int)Constants.InfoBoxMessageType.Warning, "Attention", "Unable to Update user, try again later. Error message: " + Utils.ModuleSecurity().LastError);
                    }
                }
                else
                {
                    if (Utils.ModuleSecurity().NewUser(nume, prenume, login, password, email, groupList, passwordStatusID, recordStatusID))
                    {
                        ...
                        FillUsersGridView();
                        userDetailsActionHiddenField.Value = Crypt.Module.CreateEncodedString(Constants.UserOperation.Edit);
                        Message(userDetailsPanel, (int)Constants.InfoBoxMessageType.Ok, "Congratulation", "Succesifuly added user information.");
                    }
                    else
                    {
                        Message(userDetailsPanel, (int)Constants.InfoBoxMessageType.Warning, "Attention", "Unable to add new user, try again later. Error message: " + Utils.ModuleSecurity().LastError);
                    }
                }
            }
            else
            {
                Message(userDetailsPanel, (int)Constants.InfoBoxMessageType.Warning, "Attention", "Any parameters is not valid, please ask for help from system administrator!");
            }
        }
        else
        {
